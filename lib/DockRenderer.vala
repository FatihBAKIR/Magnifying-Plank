	...
	public class DockRenderer : AnimatedRenderer
	{
		...
		public void draw_dock (Context cr)
		{
...
#endif
#endif
			DockItem hovered_item = controller.window.HoveredItem;
			...
			if (hovered_item != null)
				hoverIndex = (short)items.index_of(hovered_item);
...
#endif
				if (hovered_item != null && hovered_item == item)				
					continue;				

				if (dragged_item != item)
					draw_item (item, hoverIndex - i);
#if BENCHMARK
		...
		void draw_dock_background ()
		{
			...
			//Logger.notification(background_rect.width.to_string());
		...
		void draw_item (DockItem item, short dist)
		{
			...
			TimeSpan hover_time = (int)Math.fmin(frame_time.difference (item.LastHovered), 150000);;
			double size_mul = Math.fmax(0, ((double)hover_time / 150000));
			dist = (short)Math.fabs(dist);
			size_mul += 1;
			int xoff = 0, yoff = 0;
			if (magnify)
			{
				xoff = (int)((icon_size / 2) * (size_mul - 1));
				yoff = (int)(icon_size * (size_mul - 1));
				icon_size = (int)(icon_size * size_mul);
				shadow_size = (int)(shadow_size * size_mul);
			}
			else if (hovering)
			{
				if (side)				
			...
			if (icon_shadow_surface != null) {
				...
				shadow_cr.set_source_surface (icon_shadow_surface.Internal, 
					(draw_value.draw_region.x - shadow_size) * window_scale_factor - xoff, 
					(draw_value.draw_region.y - shadow_size) * window_scale_factor - yoff);
				shadow_cr.paint ();
				if (window_scale_factor > 1)
					shadow_cr.restore ();
			}
			...
			main_cr.set_source_surface (icon_surface.Internal, draw_value.draw_region.x * window_scale_factor - xoff, draw_value.draw_region.y * window_scale_factor - yoff);
			main_cr.paint ();
			...
			if (item.Indicator != IndicatorState.NONE)
				draw_indicator_state (draw_value.hover_region, item.Indicator, item.State, xoff);
		}
		...
		void draw_indicator_state (Gdk.Rectangle item_rect, IndicatorState indicator, ItemState item_state, int xoff)
		{
			...
			default:
			case PositionType.BOTTOM:
				x = item_rect.x - xoff + item_rect.width / 2.0 - indicator_surface.Width / 2.0;
